...
9. [Secret Management (Vault)](#secret-management-vault)
...
> **Full K8s documentation:** See [K8S-INFRASTRUCTURE.md](K8S-INFRASTRUCTURE.md) for complete Kubernetes setup including Istio service mesh, mTLS, canary deployments, circuit breakers, authorization policies, and operational procedures.

---

## Kubernetes Deployment

For the complete Kubernetes infrastructure guide including:
- Istio service mesh configuration (mTLS, traffic management, circuit breakers)
...
See the dedicated document: **[K8S-INFRASTRUCTURE.md](K8S-INFRASTRUCTURE.md)**
...
# 2. Apply namespace with Istio injection
kubectl apply -f interview-platform-backend/k8s/istio/namespace.yaml

# 3. Create secrets
kubectl create secret generic db-credentials \
  --from-literal=url='jdbc:postgresql://db:5432/interview_platform' \
...
kubectl create secret generic jwt-secrets \
  ...
  --from-literal=refresh-secret="$(openssl rand -base64 48)" \
  -n interview-platform
...
kubectl apply -f interview-platform-backend/k8s/istio/

# 6. Verify
kubectl get pods -n interview-platform
istioctl analyze -n interview-platform
```
